...
var _Sources = (() => {
  ...
  var require_SourceInfo = __commonJS({
    "node_modules/@paperback/types/lib/base/SourceInfo.js"(exports) {
      ...
      (function(ContentRating4) {
        ContentRating4["EVERYONE"] = "EVERYONE";
        ContentRating4["MATURE"] = "MATURE";
        ContentRating4["ADULT"] = "ADULT";
      })(ContentRating3 = exports.ContentRating || (exports.ContentRating = {}));
    }
  });
  ...
  var ReadComicsOnline_exports = {};
  __export(ReadComicsOnline_exports, {
  ...
  var Requests = class {
    ...
    async getChapterPage(comicId, chapterId) {
     ...
     */
    async getAdvancedSearchToken() {
      const html3 = await this.getAdvancedSearchPage();
      const match = /_token:\s*'([^']+)'/.exec(html3);
      if (!match?.[1]) {
        throw new Error("Unable to find the advanced search CSRF token");
      }
      return match[1];
    }
    async advancedSearch(page, categoryIds) {
      const token = await this.getAdvancedSearchToken();
      const innerPairs = categoryIds.map((id) => ["categories[]", id]);
  ...
  function parseChapterDate(text3) {
    ...
    if (/today/i.test(cleaned) || !cleaned) return /* @__PURE__ */ new Date();
  ...
  var Parsers = class {
    ...
    parseComicGrid(html3) {
      ...
      $2(".media").each((_, el) => {
        ...
        $el.find(".media-body > div").each((__, div) => {
          ...
          const text3 = $div.text().trim();
          if (text3) subtitle = text3;
        });
        items.push({
          id,
    ...
    parseSearchSuggestions(suggestions) {
      return suggestions.map((suggestion) => ({
        id: suggestion.data,
        title: load(suggestion.value).text().trim(),
        imageUrl: ""
      }));
    }
    toPartialSourceMangas(items) {
      return items.map(
        (item) => App.createPartialSourceManga({
          mangaId: item.id,
          title: item.title,
          image: item.imageUrl,
    ...
    parseLatestReleases(html3) {
      const $2 = load(html3);
      const results = [];
      $2(".manga-item").each((_, el) => {
        const $el = $2(el);
    ...
    parseComicDetails(html3) {
      const $2 = load(html3);
      let author;
      let status;
      let type;
      const genres = [];
      $2(".dl-horizontal dt").each((_, dt) => {
        const $dt = $2(dt);
      ...
      $2("dd.tag-links a").each((_, a) => {
        const $a = $2(a);
        const slug = ($a.attr("href") ?? "").split("/").pop() ?? "";
        if (slug) genres.push({ id: slug, value: $a.text().trim() });
      });
      const ratingAttr = $2("[data-score]").first().attr("data-score");
      return {
        title: $2(".listmanga-header").first().text().replace(/Chapters$/i, "").trim(),
        imageUrl: absoluteImageUrl($2(".boxed img").first().attr("src") ?? ""),
        synopsis: $2(".manga.well p").first().text().trim(),
        author,
        status,
        type,
        genres,
        rating: ratingAttr ? Number(ratingAttr) : void 0
      };
    }
    parseChapterList(html3) {
      const $2 = load(html3);
      const entries = [];
      $2(".chapters > li").each((_, li) => {
        const $li = $2(li);
    ...
    parseChapterPages(html3) {
      const $2 = load(html3);
      return $2("#all .imagecnt img").map((_, img) => $2(img).attr("data-src") ?? $2(img).attr("src") ?? "").get().map((src) => absoluteImageUrl(src)).filter((src) => src.length > 0);
    }
    buildMangaDetails(comicId, details) {
      ...
      return App.createSourceManga({
        ...
        mangaInfo: App.createMangaInfo({
          ...
          tags: tagSections,
          additionalInfo: details.type ? { Type: details.type } : void 0
        })
      });
    }
    buildChapters(entries) {
      return entries.map(
        (entry, index2) => App.createChapter({
          id: entry.id,
    ...
    buildChapterDetails(mangaId, chapterId, pages) {
      return App.createChapterDetails({ id: chapterId, mangaId, pages });
    }
    buildLatestUpdatesHomeSection(id, title, entries, containsMoreItems) {
      return App.createHomeSection({
        ...
        containsMoreItems,
        items: entries.map(
          ({ manga }) => App.createPartialSourceManga({
            mangaId: manga.id,
            title: manga.title,
  ...
  var FilterPreferences = class {
    ...
    extractOptions($2, selectName) {
      const options = [];
      $2(`select[name="${selectName}"] option`).each((_, el) => {
        const id = $2(el).attr("value");
        const value = $2(el).text().trim();
        if (id) options.push({ id, value });
      });
      return options;
    }
    async populateFilters(stateManager, fetchAdvancedSearchPage, force = false) {
      const lastFetch = Number(await stateManager.retrieve("last-filter-fetch") ?? 0);
      const cached = lastFetch + FILTER_CACHE_SECONDS > Date.now() / 1e3;
      if (cached && !force) {
        const categories = await stateManager.retrieve(".categories");
        if (categories) {
          this.categoryFilter = categories;
          return;
        }
      }
      const html3 = await fetchAdvancedSearchPage();
      const $2 = load(html3);
      this.categoryFilter = this.extractOptions($2, "categories[]");
      await stateManager.store(".categories", this.categoryFilter);
      await stateManager.store("last-filter-fetch", Date.now() / 1e3);
    }
  };
  ...
  var ReadComicsGeneric = class extends import_types5.Source {
    ...
    async getChapters(mangaId) {
      const html3 = await this.requests.getComicDetails(mangaId);
      const entries = parsers.parseChapterList(html3);
      return parsers.buildChapters(entries);
    }
    async getChapterDetails(mangaId, chapterId) {
      const html3 = await this.requests.getChapterPage(mangaId, chapterId);
      const pages = parsers.parseChapterPages(html3);
      return parsers.buildChapterDetails(mangaId, chapterId, pages);
    }
    async getSearchTags() {
      await filter4.populateFilters(this.stateManager, () => this.requests.getAdvancedSearchPage());
      return [
        App.createTagSection({
          id: "categories",
          label: "Categories",
          tags: filter4.getCategoryFilter().map((category) => App.createTag({ id: category.id, label: category.value }))
        })
      ];
    }
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query.title) {
        ...
        const items2 = parsers.parseSearchSuggestions(suggestions);
        return parsers.toPagedResults(items2, void 0);
      }
      ...
      const html3 = includedCategoryIds.length > 0 ? await this.requests.advancedSearch(page, includedCategoryIds) : await this.requests.getComicList(page);
      const items = parsers.parseComicGrid(html3).filter((item) => !item.subtitle || !excludedNames.has(item.subtitle));
      return parsers.toPagedResults(items, items.length > 0 ? { page: page + 1 } : void 0);
    }
  ...
  var ReadComicsOnline = new ReadComicsOnlineExtension();
